    ...
    for (const card of cardsToProcess) {
      try {
        ...
        const data = await card.evaluate((el: Element) => {
          ...
          const priceEl = el.querySelector('.app-vendor-tile-price');
          const capacityEl = el.querySelector('.app-vendor-tile-capacity');
          ...
          const sliderImages = Array.from(el.querySelectorAll('.vendorTileGallery__image'))
            .map(img => {
              const dataSrc = img.getAttribute('data-src');
              const src = img.getAttribute('src');
              const srcset = img.getAttribute('data-srcset');
              return dataSrc || srcset?.split(',')[0]?.trim()?.split(' ')[0] || src;
            })
            .filter(Boolean);
          ...
          console.log('images trouvées:', sliderImages);
