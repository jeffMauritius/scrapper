...
async function scrapeListPage(page: Page): Promise<VenueData[]> {
  ...
  const ITEMS_PER_PAGE = 24; // Nombre d'établissements par page
  const START_ITEM = 6501;
  const END_ITEM = 8338;
  const START_PAGE = Math.floor(START_ITEM / ITEMS_PER_PAGE) + 1;
  const END_PAGE = Math.ceil(END_ITEM / ITEMS_PER_PAGE);
  
  console.log(`\nDébut du traitement des établissements ${START_ITEM} à ${END_ITEM}`);
  console.log(`Pages à traiter : ${START_PAGE} à ${END_PAGE}\n`);
  ...
  try {
    for (let currentPage = START_PAGE; currentPage <= END_PAGE; currentPage++) {
      ...
      console.log(`\nNavigation vers la page ${currentPage}/${END_PAGE}...`);
      ...
      for (let i = 0; i < cards.length; i++) {
        const card = cards[i];
        try {
          console.log(`\nTraitement de la carte ${i + 1}/${cards.length} (page ${currentPage}/${END_PAGE})...`);
          ...
          if (venues.length % 100 === 0) {
            console.log(`\n=== ${venues.length} établissements traités sur ${END_ITEM} ===`);
      ...
      if (currentPage < END_PAGE) {
        const waitTime = Math.random() * 3000 + 2000; // Entre 2 et 5 secondes
...
async function saveData(venues: VenueData[]) {
  try {
    ...
    let existingData = { venues: [] as VenueData[] };
    ...
    // Filtrer les nouveaux établissements pour éviter les doublons
    const newVenues = venues.filter(newVenue => {
      // Vérifier si l'établissement existe déjà (même nom et même ville)
      const exists = existingData.venues.some(existingVenue => 
        existingVenue.name.toLowerCase().trim() === newVenue.name.toLowerCase().trim() &&
