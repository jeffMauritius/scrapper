...
async function scrapeListPage(page: Page): Promise<VenueData[]> {
  ...
  try {
    ...
    console.log(`${cards.length} cartes trouvées, traitement des 3 premières pour le développement`);

    // Limiter aux 3 premières cartes pour le développement
    const cardsToProcess = cards.slice(0, 3);

    for (const card of cardsToProcess) {
      try {
        console.log('\nAnalyse d\'une nouvelle carte...');
        ...
        const data = await card.evaluate((el: Element) => {
          ...
          const nameEl = el.querySelector('.vendorTile__title, .app-vendor-tile-title, .storefront-header-title');
          ...
          const priceEl = el.querySelector('.vendorTile__price, .vendor-price, .storefront-price, [data-precio]');
          const capacityEl = el.querySelector('.vendorTile__capacity, .vendor-capacity, .storefront-capacity, [data-capacidad]');
          const descriptionEl = el.querySelector('.vendorTile__description, .vendor-description, .storefront-description');
          const linkEl = el.querySelector('a.vendorTile__link, a[href*="/"], .storefront-link') as HTMLAnchorElement;
          const imageEl = el.querySelector('.vendorTile__image img, .vendor-image img, .storefront-image img, [data-src]') as HTMLImageElement;
          const typeEl = el.querySelector('.vendorTile__subtitle, .vendor-type, .storefront-type');

          // Logger chaque élément pour déboguer avec plus de détails
          const logElement = (name: string, el: Element | null) => {
            if (el) {
              console.log(`${name}:`, {
          ...
          logElement('price', priceEl);
          logElement('capacity', capacityEl);
          logElement('description', descriptionEl);
          logElement('link', linkEl);
          logElement('image', imageEl);
          logElement('type', typeEl);

          // Extraire le prix depuis l'attribut data si disponible
          const priceFromData = priceEl?.getAttribute('data-precio') || '';
          const capacityFromData = capacityEl?.getAttribute('data-capacidad') || '';
          ...
          return {
            ...
            price: priceFromData || priceEl?.textContent?.trim() || '',
            capacity: capacityFromData || capacityEl?.textContent?.trim() || '',
            description: descriptionEl?.textContent?.trim() || '',
            url: linkEl?.href || '',
            image: imageEl?.getAttribute('data-src') || imageEl?.src || '',
