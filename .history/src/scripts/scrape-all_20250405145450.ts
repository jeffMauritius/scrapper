...
async function scrapeListPage(page: Page, baseUrl: string): Promise<string[]> {
  ...
  // Attendre et vérifier la présence des éléments
  try {
  ...
  console.log(`Nombre d'éléments trouvés après filtrage: ${uniqueLinks.length}`);
  console.log('Premiers liens trouvés:', uniqueLinks.slice(0, 3));
