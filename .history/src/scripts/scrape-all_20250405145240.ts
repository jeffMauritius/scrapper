...
async function scrapeListPage(page: Page, baseUrl: string): Promise<string[]> {
  ...
  await page.waitForTimeout(5000); // Attendre 5 secondes supplémentaires pour le chargement dynamique

  // Attendre que les éléments soient chargés
  await page.waitForSelector('.app-catalog-list-directory', { timeout: 60000 });
  
  // Extraire les URLs
  const links = await page.$$eval('.app-catalog-list-directory article a[href*="/chateau-mariage/"]', (elements) => {
    return elements
