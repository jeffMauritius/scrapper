...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const name = await getTextContent('h1.storefront-header-title, h1.appHeading, .storefront__title', 'Nom');
    const type = await getTextContent('.storefront-header-info, .vendorInfo, .storefront__subtitle', 'Type');
    const description = await getTextContent('.storefront-description, .vendorDescription, .storefront__description', 'Description');
    const price = await getTextContent('.storefront-header-price, .vendorPrice, .storefront__price', 'Prix');
    const address = await getTextContent('.storefront-header-address, .vendorAddress, .storefront__address', 'Adresse');
    const capacity = await getTextContent('.storefront-header-capacity, .vendorCapacity, .storefront__capacity', 'Capacité');
    const rating = await getTextContent('.storefront-header-rating, .vendorRating, .storefront__rating', 'Note');
    const images = await getImages('.storefront-gallery img, .vendorGallery img, .storefront__gallery img, .gallery-slider img');
