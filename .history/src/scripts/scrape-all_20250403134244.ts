...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const name = await page.textContent(SELECTORS.venueName) || '';
    const type = await page.textContent(SELECTORS.venueType) || '';
    const description = await page.textContent(SELECTORS.venueDescription) || '';
    const price = await page.textContent(SELECTORS.venuePrice) || '';
    const address = await page.textContent(SELECTORS.venueAddress) || '';
    const capacity = await page.textContent(SELECTORS.venueCapacity) || '';
    const rating = await page.textContent(SELECTORS.venueRating) || '';
    
    // Extraire les images
    const images = await page.$$eval(SELECTORS.venueImages, (imgs) => 
