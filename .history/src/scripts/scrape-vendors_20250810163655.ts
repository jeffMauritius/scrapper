      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        try {
          ...
          const data = await card.evaluate((el: Element) => {
            ...
            // Nouveaux sélecteurs pour les vidéographes - testons plusieurs variantes
            const nameSelectors = [
              '.storefront-name', 
              '.vendorTile__title', 
              '.businessCard__title', 
            ...
            for (const selector of reviewsSelectors) {
              const el = el.querySelector(selector);
              if (el && el.textContent?.trim()) {
                reviews = el.textContent.trim().replace(/[()]/g, '');
            ...
            for (const selector of linkSelectors) {
              const el = el.querySelector(selector) as HTMLAnchorElement;
              if (el && el.href) {
                url = el.href;
...
async function saveData(vendors: VendorData[]) {
  try {
    const outputPath = path.join(process.cwd(), 'data/videographers.json');
    ...
    if (fs.existsSync(outputPath)) {
      try {
        ...
        console.log('Données existantes chargées:', existingData.vendors.length, 'vidéographes');
      } catch (error) {
        console.log('Erreur lors de la lecture du fichier existant, création d\'un nouveau fichier');
      }
    }

    // Filtrer les nouveaux vidéographes pour éviter les doublons
    const newVendors = vendors.filter(newVendor => {
      ...
      if (exists) {
        console.log(`⚠️ Vidéographe déjà existant, ignoré: ${newVendor.name} à ${newVendor.city}`);
        return false;
      }
      console.log(`✓ Nouveau vidéographe à ajouter: ${newVendor.name} à ${newVendor.city}`);
      console.log('Images:', newVendor.images);
      return true;
    });

    if (newVendors.length === 0) {
      console.log('Aucun nouveau vidéographe à ajouter');
      return;
    }
    ...
    console.log(`${newVendors.length} nouveaux vidéographes ajoutés, total: ${existingData.vendors.length}`);
  } catch (error) {
...
async function main() {
  ...
  try {
    console.log('Démarrage du scraping des vidéographes...');
    
    // Visiter la page d'accueil pour initialiser la session
    console.log('Visite de la page d\'accueil des vidéographes pour initialiser la session...');
    await page.goto('https://www.mariages.net/video-mariage', { waitUntil: 'networkidle' });
    await page.waitForTimeout(5000);

    // Scraper les vidéographes
