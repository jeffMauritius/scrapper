...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const name = await getTextContent('h1.storefrontTitle', 'Nom');
    const type = await getTextContent('.storefrontTypes', 'Type');
    const description = await getTextContent('.storefrontDescription', 'Description');
    const price = await getTextContent('.storefrontHeaderPrice', 'Prix');
    const address = await getTextContent('.storefrontHeaderAddress', 'Adresse');
    const capacity = await getTextContent('.storefrontFeatures .capacity', 'Capacité');
    const rating = await getTextContent('.storefrontHeaderRating', 'Note');
    const images = await getImages('.gallery-thumbs img, .storefrontGallery img');
