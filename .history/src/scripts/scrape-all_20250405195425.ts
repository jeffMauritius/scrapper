...
async function saveData(venues: VenueData[]) {
  try {
    // Correction du chemin pour pointer vers le dossier data à la racine
    const outputPath = path.join(process.cwd(), 'data/venues.json');
    
    // Lire les données existantes
    let existingData: VenueData[] = [];
    ...
    const allData = [...existingData, ...venues];
    ...
    console.log(`${venues.length} nouveaux établissements, total: ${allData.length}`);
  } catch (error) {
