...
async function scrapeListPage(page: Page): Promise<string[]> {
  ...
  try {
    ...
    await page.waitForTimeout(10000); // Augmenter le délai d'attente
    console.log('Recherche des éléments...');

    // Afficher le HTML pour debug
    console.log('\nAnalyse de la structure de la page...');
    const html = await page.content();
    console.log(html.substring(0, 1000)); // Afficher le début du HTML

    // Liste étendue de sélecteurs à essayer
    const selectors = [
      '.directory-list-item',
      '.app-directory-list-item',
      '.listingCard',
      '.vendorTile',
      '.app-catalog-list-directory',
      '.storefront-list',
      '.vendor-list',
      'article[data-type="business"]',
      '.business-card',
      ...
      'article',
      '.card',
      '[data-href*="/chateau-mariage"]',
      '[data-href*="/domaine-mariage"]',
      'a[href*="/chateau-mariage"]',
      'a[href*="/domaine-mariage"]'
    ];

    console.log('\nTest des sélecteurs disponibles:');
    
    // Vérifier tous les éléments cliquables
    const allClickable = await page.$$eval('a[href], [data-href]', elements => {
      return elements.map(el => {
        const href = el.getAttribute('href') || el.getAttribute('data-href');
        return { href, text: el.textContent?.trim() };
      }).filter(item => item.href && (
        item.href.includes('mariage') || 
        item.href.includes('reception') ||
        item.href.includes('chateau') ||
        item.href.includes('domaine')
      ));
    });

    console.log('\nÉléments cliquables trouvés:', allClickable.length);
    console.log('Exemples de liens:', allClickable.slice(0, 3));
    ...
    for (const selector of selectors) {
      try {
        ...
        if (count > 0) {
          // Vérifier si ces éléments contiennent des liens pertinents
          const links = await page.$$eval(`${selector} a[href], ${selector} [data-href]`, elements => {
            return elements.map(el => {
              const href = el.getAttribute('href') || el.getAttribute('data-href');
              return href;
            }).filter(Boolean);
          ...
          if (links.length > 0) {
            console.log(`Liens trouvés dans "${selector}":`, links.length);
        ...
        if (debug.fixed && debug.solution) {
          foundSelector = debug.solution.split('"')[3];
          break;
        }
      }
    }

    // Si aucun sélecteur ne fonctionne, essayer d'extraire tous les liens pertinents
    if (!foundSelector && allClickable.length > 0) {
      console.log('\nUtilisation de la méthode de secours pour extraire les liens...');
      allLinks = allClickable
        .map(item => item.href)
        .filter(href => href && (
          href.includes('/chateau-mariage') ||
          href.includes('/domaine-mariage') ||
          href.includes('/salle-mariage') ||
          href.includes('/hotel-mariage')
        ))
        .map(href => href.startsWith('http') ? href : `https://www.mariages.net${href}`);
    }

    // Si toujours rien trouvé, lancer le débogage
    if (allLinks.length === 0) {
      console.log('\nAucun lien trouvé avec les méthodes standards, analyse approfondie...');
      
      // Rechercher tous les éléments qui pourraient être des conteneurs
      const potentialContainers = await page.$$eval('div, section, article', elements => {
        return elements
          .filter(el => el.children.length > 5) // Conteneurs avec plusieurs enfants
          .map(el => ({
            id: el.id,
      ...
      console.log('Conteneurs potentiels trouvés:', potentialContainers);
      await autoDebug(page, 'Analyse structurelle terminée', 'Vérification finale');
    }
