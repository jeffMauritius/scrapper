...
async function getFullDescription(page: Page): Promise<string> {
  try {
    // Vérifier si le bouton "Lire plus" existe
    const readMoreButton = await page.$('.app-storefront-description-read-more-button');
    if (readMoreButton) {
        ...
        if (data.url) {
          ...
          try {
            ...
            const venue: VenueData = {
              ...
              name: data.name,
              type,
              description: fullDescription || data.description, // Utiliser la description complète si disponible
              images,
              price: data.price,
            ...
            console.log('Description complète:', venue.description.substring(0, 200) + '...');
