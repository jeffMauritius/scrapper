...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const type = await getTextContent('#layoutMain > article > header > h1', 'Type');
    ...
    const images = await getImages('body > div.modalFullscreen > div > section > div.app-gallery-mosaic-fullscreen-slider.storefrontGalleryMosaicFullscreen__content > div > div:nth-child(1) > figure > picture');
