...
async function scrapeVenuePage(page: puppeteer.Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const name = await page.$eval('.vendorTitle h1, .storefront-header-title', (el: Element) => el.textContent?.trim() || '').catch(() => '');
    console.log('Nom trouvé:', name);

    const type = await page.$eval('.venueInfo-tags-item, .storefront-header-tag', (el: Element) => el.textContent?.trim() || '').catch(() => '');
    console.log('Type trouvé:', type);

    const description = await page.$eval('.vendorDescription, .storefront-description', (el: Element) => el.textContent?.trim() || '').catch(() => '');
    console.log('Description trouvée:', description ? 'Oui' : 'Non');

    // Extraire les images
    const images = await page.$$eval(
      '.gallery-slider img, .storefront-gallery img, .vendor-gallery img',
      (elements: Element[]) => elements.map((el: Element) => el.getAttribute('data-src') || el.getAttribute('src'))
        .filter((url: string | null): url is string => url !== null && !url.includes('blank.gif'))
    ).catch(() => []);
    console.log(`Nombre d'images trouvées: ${images.length}`);

    // Extraire le prix
    const priceText = await page.$eval('.app-price-lead, .storefront-price', (el: Element) => el.textContent?.trim() || '').catch(() => '');
    ...
    // Extraire l'adresse
    const address = {
      city: await page.$eval('.venue-address, .storefront-address-city', (el: Element) => el.textContent?.trim() || '').catch(() => ''),
      region: await page.$eval('.venue-region, .storefront-address-region', (el: Element) => el.textContent?.trim() || '').catch(() => 'Non spécifié'),
      country: 'France'
    };
    console.log('Adresse trouvée:', address);

    // Extraire la capacité
    const capacityText = await page.$eval('.capacity-block, .storefront-capacity', (el: Element) => el.textContent?.trim() || '').catch(() => '');
    ...
    const rating = {
      score: parseFloat(await page.$eval('.rating-number, .storefront-rating-score', (el: Element) => el.textContent?.trim() || '0').catch(() => '0')),
      numberOfReviews: parseInt(await page.$eval('.rating-count, .storefront-rating-count', (el: Element) => el.textContent?.trim() || '0').catch(() => '0'))
    };
