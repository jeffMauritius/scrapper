    ...
    for (const card of cardsToProcess) {
      try {
        ...
        const data = await card.evaluate((el: Element) => {
          const log = (selector: string, element: Element | null) => {
          ...
          const nameEl = el.querySelector('.vendorTile__title, .app-vendor-tile-title');
          const ratingEl = el.querySelector('.vendorTile__rating, .rating-badge');
          const reviewsEl = el.querySelector('.rating-counter, .reviewCount');
          const locationEl = el.querySelector('.vendorTile__location, .vendor-location');
          const priceEl = el.querySelector('.vendorTile__price, .vendor-price');
          const capacityEl = el.querySelector('.vendorTile__capacity, .vendor-capacity');
          const descriptionEl = el.querySelector('.vendorTile__description, .vendor-description');
          const linkEl = el.querySelector('a.vendorTile__link, a[href*="/"]') as HTMLAnchorElement;
          const imageEl = el.querySelector('.vendorTile__image img, .vendor-image img') as HTMLImageElement;
          const typeEl = el.querySelector('.vendorTile__subtitle, .vendor-type');

          // Logger chaque élément pour déboguer
          log('name', nameEl);
          log('rating', ratingEl);
          log('reviews', reviewsEl);
          log('location', locationEl);
          log('price', priceEl);
          log('capacity', capacityEl);
