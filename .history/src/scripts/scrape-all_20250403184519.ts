...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const name = await getTextContent('h1', 'Nom');
    const type = await getTextContent('.app-emp-info-category', 'Type');
    const description = await getTextContent('.app-emp-description', 'Description');
    const price = await getTextContent('.app-emp-info-price', 'Prix');
    const address = await getTextContent('.app-emp-info-address', 'Adresse');
    const capacity = await getTextContent('.app-emp-info-capacity', 'Capacité');
    const rating = await getTextContent('.app-emp-info-rating', 'Note');
    const images = await getImages('.app-emp-gallery img');
