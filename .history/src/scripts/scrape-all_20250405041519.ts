...
async function scrapeVenuePage(page: Page, url: string): Promise<VenueData | null> {
  try {
    ...
    const address = await getTextContent('a.app-heading-quick-link', 'Adresse');
    ...
    const images = await getImages('storefrontMultiGallery__scroll app-gallery-slider-container app-scroll-snap-container ');
