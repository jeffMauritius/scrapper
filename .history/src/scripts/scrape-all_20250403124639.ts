...
const PAGE_OPTIONS: puppeteer.WaitForOptions & puppeteer.GoToOptions = {
...
async function scrapeListPage(page: puppeteer.Page, url: string): Promise<string[]> {
  try {
    ...
    console.log('Contenu HTML de la page:', html.substring(0, 1000) + '...');
        ...
        const links = await page.$$eval(
          ...
          (elements: Element[]) => {
            return elements
              ...
              .map(el => {
                console.log('Lien trouvé:', el.href);
                return el.href;
              })
              .filter(href => href && (
                href.includes('/salle/') || 
