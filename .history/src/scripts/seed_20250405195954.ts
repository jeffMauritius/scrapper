    ...
    for (const venue of data.venues) {
      ...
      const establishment = await prisma.establishment.create({
        data: {
          ...
          country: 'FR',
          minCapacity: null,
          maxCapacity: venue.capacity ? parseInt(venue.capacity) || null : null,
          rating: venue.rating ? parseFloat(venue.rating.split(' ')[0]) || null : null,
          reviewCount: venue.rating ? parseInt(venue.rating.match(/\((\d+)\)/)?.[1] || '0') : 0,
          featureIds: [],
          amenityIds: [],
          images: {
