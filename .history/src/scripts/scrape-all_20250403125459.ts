...
async function scrapeListPage(page: any, url: string): Promise<string[]> {
  try {
    ...
    console.log('HTML de la page:', html);
    ...
    const selectors = [
      ...
      '.app-vendor-tile a',
      '.vendorTile__link',
      '.app-directory-list-item__link',
      '.app-vendor-tile__link',
      '.vendorTile a[href*="/salle/"]',
      '.vendorTile a[href*="/restaurant/"]',
      '.vendorTile a[href*="/traiteur/"]',
      'a[href*="/salle/"]',
      'a[href*="/restaurant/"]',
      'a[href*="/traiteur/"]',
      'a[href*="/lieu/"]',
    ...
    for (const selector of selectors) {
      try {
        ...
        console.log(`Nombre d'éléments trouvés avec ${selector}:`, elements.length);
        ...
        if (elements.length > 0) {
          ...
          const filteredLinks = links.filter((href): href is string => 
            href !== null && (
              href.includes('/salle/') || 
              href.includes('/restaurant/') || 
              href.includes('/traiteur/') ||
              href.includes('/lieu/') ||
              href.includes('/domaine/') ||
              href.includes('/chateau/')
            )
          );
          
          if (filteredLinks.length > 0) {
            console.log(`${filteredLinks.length} liens valides trouvés avec le sélecteur ${selector}`);
            allLinks = [...allLinks, ...filteredLinks];
          }
        }
      } catch (error) {
