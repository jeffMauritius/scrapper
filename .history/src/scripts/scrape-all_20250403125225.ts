...
async function waitForSelectorWithFallback(page: any, selectors: string[]): Promise<string> {
...
async function scrapeVenuePage(context: any, url: string): Promise<VenueData | null> {
  ...
  try {
    ...
    const name = await page.locator(nameSelector).textContent();
    console.log('Name:', name);

    const typeSelector = await waitForSelectorWithFallback(page, COMMON_SELECTORS.type);
    const type = await page.locator(typeSelector).textContent();
    console.log('Type:', type);

    const descriptionSelector = await waitForSelectorWithFallback(page, COMMON_SELECTORS.description);
    const description = await page.locator(descriptionSelector).textContent();
    console.log('Description:', description);

    const imagesSelector = await waitForSelectorWithFallback(page, COMMON_SELECTORS.images);
    const images = await page.locator(imagesSelector).evaluateAll((elements: Element[]) => 
      elements
        .map(el => (el as HTMLImageElement).getAttribute('data-src') || (el as HTMLImageElement).getAttribute('src'))
    ...
    const priceText = await page.locator(priceSelector).textContent();
    ...
    const addressText = await page.locator(addressSelector).textContent();
    ...
    const capacityText = await page.locator(capacitySelector).textContent();
    ...
    const ratingText = await page.locator(ratingSelector).textContent();
    const ratingValue = parseFloat(ratingText || '0') || 0;
    const rating = {
      score: ratingValue,
      numberOfReviews: 0
    };
...
async function scrapeListPage(page: any, url: string): Promise<string[]> {
  try {
    ...
    console.log('Capture d\'écran sauvegardée dans debug-list.png');
    
    // Essayer différents sélecteurs pour les liens
    const selectors = [
      '.vendorList .storefront-link',
      '.vendorList .vendor-tile-link',
      '.vendorList .app-vendor-tile-link',
      '.vendorList a[href*="/salle/"]',
      '.vendorList a[href*="/restaurant/"]',
      '.vendorList a[href*="/traiteur/"]',
      '.storefront-list .storefront-link',
      '.storefront-list a[href*="/salle/"]',
      '.storefront-list a[href*="/restaurant/"]',
      '.storefront-list a[href*="/traiteur/"]',
      '.directory-list .directory-item-link',
      '.directory-list a[href*="/salle/"]',
      '.directory-list a[href*="/restaurant/"]',
      '.directory-list a[href*="/traiteur/"]'
    ];
    ...
    for (const selector of selectors) {
      try {
        ...
        const links = await page.locator(selector).evaluateAll((elements: Element[]) => {
          return elements
            ...
            .filter(href => href && (
              ...
              href.includes('/traiteur/')
            ));
        });
        
        if (links.length > 0) {
          console.log(`${links.length} liens trouvés avec le sélecteur ${selector}`);
...
async function main() {
  ...
  try {
    ...
    const venueLinks = await scrapeListPage(page, 'https://www.mariages.net/busc.php?id_grupo=1&id_sector=31&showmode=list&NumPage=1&userSearch=1&isNearby=0&priceType=menu');
    console.log(`${venueLinks.length} liens de lieux trouvés`);

    for (const url of venueLinks) {
      console.log(`\nTraitement du lieu: ${url}`);
      const venueData = await scrapeVenuePage(context, url);
