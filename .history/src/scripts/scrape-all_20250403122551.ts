...
async function scrapeListPage(page: puppeteer.Page, url: string): Promise<string[]> {
  try {
    ...
    // Simuler un comportement humain avant de scraper
    await simulateHumanBehavior(page);
    
    const links = await page.$$eval(
      COMMON_SELECTORS.venueLinks,
      (elements: Element[]) => {
        return elements
          .filter((el): el is HTMLAnchorElement => el instanceof HTMLAnchorElement)
          .map(el => el.href)
          .filter(href => href && href.includes('/reception/'));
      }
    );
...
async function main() {
  ...
  try {
    ...
    const venueLinks = await scrapeListPage(page, 'https://www.mariages.net/reception');
