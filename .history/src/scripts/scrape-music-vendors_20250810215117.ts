...
async function scrapeListPage(page: Page): Promise<VendorData[]> {
  ...
  const TOTAL_ITEMS = 8234; // Nombre de prestataires de musique
  const TOTAL_PAGES = Math.ceil(TOTAL_ITEMS / ITEMS_PER_PAGE);
  let totalProcessed = 0;

  try {
    // Commencer depuis le début pour les prestataires de musique
    for (let currentPage = 1; currentPage <= TOTAL_PAGES; currentPage++) {
      const url = `https://www.mariages.net/busc.php?id_grupo=2&id_sector=3&isNearby=0&NumPage=${currentPage}`;
      console.log(`\n=== Page ${currentPage}/${TOTAL_PAGES} (${Math.round((currentPage/TOTAL_PAGES) * 100)}%) ===`);
      console.log(`Scraping des prestataires de musique - page ${currentPage}`);
      console.log(`Total traité: ${totalProcessed} prestataires de musique`);
      ...
      // Sélecteurs spécifiques aux prestataires de musique
      const selectors = [
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        try {
          ...
          const data = await card.evaluate((el: Element) => {
            // Debug: afficher la structure HTML de la carte
            console.log('Structure HTML de la carte:', el.outerHTML.substring(0, 500));
            
            // Sélecteurs pour les prestataires de musique
            const nameSelectors = [
              '.storefront-name', 
              '.vendorTile__title', 
              '.businessCard__title', 
            ...
            const linkSelectors = [
              'a[href*="/musique-mariage/"]',
              'a[href*="/musique-mariage"]',
              'a[href*="musique"]',
              'a'
            ];

            // Essayer tous les sélecteurs pour chaque type d'information
            let name = '';
            for (const selector of nameSelectors) {
              const nameEl = el.querySelector(selector);
              if (nameEl && nameEl.textContent?.trim()) {
                name = nameEl.textContent.trim();
                console.log('Nom trouvé avec:', selector, ':', name);
                break;
              }
            }
            ...
            for (const selector of ratingSelectors) {
              ...
              if (ratingEl && ratingEl.textContent?.trim()) {
                ...
                console.log('Note trouvée avec:', selector, ':', rating);
                break;
              }
            }
            ...
            for (const selector of reviewsSelectors) {
              const reviewsEl = el.querySelector(selector);
              if (reviewsEl && reviewsEl.textContent?.trim()) {
                reviews = reviewsEl.textContent.trim().replace(/[()]/g, '');
                console.log('Avis trouvés avec:', selector, ':', reviews);
                break;
              }
            }
            ...
            for (const selector of locationSelectors) {
              ...
              if (locationEl && locationEl.textContent?.trim()) {
                ...
                console.log('Localisation trouvée avec:', selector, ':', location);
                break;
              }
            }
            ...
            for (const selector of priceSelectors) {
              ...
              if (priceEl && priceEl.textContent?.trim()) {
                ...
                console.log('Prix trouvé avec:', selector, ':', price);
                break;
              }
            }
            ...
            for (const selector of descriptionSelectors) {
              ...
              if (descEl && descEl.textContent?.trim()) {
                ...
                console.log('Description trouvée avec:', selector, ':', description);
                break;
              }
            }
            ...
            for (const selector of linkSelectors) {
              ...
              if (linkEl && linkEl.href) {
                url = linkEl.href;
                console.log('URL trouvée avec:', selector, ':', url);
                break;
              }
            }
            ...
            console.log('Données finales extraites:', {
              name, rating, reviews, location, price, description, url, images: sliderImages.length
            });
          ...
          if (data.url) {
            ...
            try {
              ...
              console.log('\n=== Données extraites pour le prestataire de musique ===');
              ...
              // Sauvegarder immédiatement après chaque prestataire
              await saveData([vendor]);
              vendors.push(vendor);
              totalProcessed++;
              console.log(`\nPrestataire de musique ajouté et sauvegardé: ${vendor.name}`);
              console.log('Services:', vendor.services);
              ...
              vendors.push(vendor);
              totalProcessed++;
              console.log(`Prestataire de musique ajouté avec données de base: ${vendor.name}`);
            } finally {
              await newPage.close();
            }
          }
          ...
          if (totalProcessed % 50 === 0) {
            console.log(`\n=== ${totalProcessed}/${TOTAL_ITEMS} prestataires de musique traités (${Math.round((totalProcessed/TOTAL_ITEMS) * 100)}%) ===`);
  ...
  console.log(`\nTotal final: ${vendors.length} prestataires de musique trouvés`);
  return vendors;
}

async function saveData(vendors: VendorData[]) {
  try {
    const outputPath = path.join(process.cwd(), 'data/music-vendors.json');
    ...
    if (fs.existsSync(outputPath)) {
      try {
        ...
        console.log('Données existantes chargées:', existingData.vendors.length, 'prestataires de musique');
      } catch (error) {
        console.log('Erreur lors de la lecture du fichier existant, création d\'un nouveau fichier');
      }
    }

    // Filtrer les nouveaux prestataires pour éviter les doublons
    const newVendors = vendors.filter(newVendor => {
      ...
      if (exists) {
        console.log(`⚠️ Prestataire de musique déjà existant, ignoré: ${newVendor.name} à ${newVendor.city}`);
        return false;
      }
      console.log(`✓ Nouveau prestataire de musique à ajouter: ${newVendor.name} à ${newVendor.city}`);
      console.log('Images:', newVendor.images);
      return true;
    });

    if (newVendors.length === 0) {
      console.log('Aucun nouveau prestataire de musique à ajouter');
      return;
    }
    ...
    console.log(`${newVendors.length} nouveaux prestataires de musique ajoutés, total: ${existingData.vendors.length}`);
  } catch (error) {
...
async function main() {
  ...
  try {
    console.log('Démarrage du scraping des prestataires de musique...');
    
    // Visiter la page d'accueil pour initialiser la session
    console.log('Visite de la page d\'accueil des prestataires de musique pour initialiser la session...');
    await page.goto('https://www.mariages.net/musique-mariage', { waitUntil: 'networkidle' });
    await page.waitForTimeout(5000);

    // Scraper les prestataires de musique
