...
async function scrapeListPage(page: Page, baseUrl: string, searchParams: string): Promise<string[]> {
  ...
  const pageUrl = `${baseUrl}${searchParams ? '?' + searchParams : ''}`;
  ...
  try {
    await page.waitForSelector('.directory-item', { timeout: 60000 });
    console.log('Sélecteur trouvé, extraction des liens...');

    // Extraire les URLs de la page courante
    const pageLinks = await page.$$eval('.directory-item a.directory-item-content', (elements) => {
      return elements
...
async function main() {
  ...
  try {
    ...
    // Ensuite, aller sur la page de recherche
    const baseUrl = 'https://www.mariages.net/reception';
    console.log('Navigation vers la page de recherche:', baseUrl);
    await page.goto(baseUrl, { waitUntil: 'networkidle' });
    await page.waitForTimeout(5000);

    // Maintenant, faire la recherche
    const searchUrl = 'https://www.mariages.net/reception/france';
