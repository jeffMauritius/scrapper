...
async function main() {
  try {
    ...
    const data = await fs.readFile(path.join(process.cwd(), 'data/venues.json'), 'utf8');
    const venuesData = JSON.parse(data) as VenueData;
    const venues = Array.isArray(venuesData.venues) ? venuesData.venues : venuesData;

    // Sélectionner les établissements 4001 à 5000
    const venuesToImport = venues.slice(4000, 5000);
    console.log(`Import des établissements 4001 à 5000 sur ${venues.length} disponibles...`);
