...
async function scrapeListPage(page: Page, baseUrl: string): Promise<string[]> {
  let allLinks: string[] = [];
  let currentPage = 1;
  const totalPages = 53; // 1269/24 arrondi au supérieur
  ...
  while (currentPage <= totalPages) {
    ...
    try {
      ...
      console.log(`Page ${currentPage}: ${uniquePageLinks.length} liens trouvés (Total: ${allLinks.length})`);
  ...
  console.log(`\nTotal final: ${finalLinks.length} châteaux trouvés sur ${currentPage-1} pages`);
...
async function main() {
  ...
  try {
    ...
    const baseUrl = 'https://www.mariages.net/chateau-mariage/france?numberOfResults=1269&page=';
