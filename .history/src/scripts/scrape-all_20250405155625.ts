  ...
  try {
    ...
    for (const card of cards) {
      try {
        ...
        // Extraire la ville et la région de la localisation
        const [city, region] = data.location.split(',').map(s => s.trim());

        // Déterminer le type en fonction de l'URL
        const type = determineVenueType(data.url);
        ...
        const venue: VenueData = {
          ...
          images: [], // On remplira ça plus tard
          price: data.price,
        ...
        // Si on a une URL valide, on va chercher les images
        if (venue.url) {
          console.log(`\nExtraction des images pour ${venue.name}...`);
          
          // Ouvrir la page du lieu dans un nouvel onglet
          const newPage = await page.context().newPage();
          try {
            await newPage.goto(venue.url, PAGE_OPTIONS);
            ...
            const images = await newPage.$$eval('.gallery-thumbs img, .gallery-slider img', (imgs) => {
              return imgs
                ...
                .filter((src, index, self) => self.indexOf(src) === index); // Dédupliquer
            });
        ...
        console.log('Erreur lors de l\'extraction des données de la carte:', error);
      }
    }
