    ...
    for (const card of cardsToProcess) {
      try {
        ...
        const data = await card.evaluate((el: Element) => {
          ...
          const priceContainerEl = el.querySelector('.vendorTilePrice');
          const priceEl = priceContainerEl?.querySelector('span:last-child');
          
          const capacityContainerEl = el.querySelector('.vendorTileFeatures');
          ...
          const sliderImages = Array.from(el.querySelectorAll('.gallery-media img, .vendorTileGallery__image, .slider-image'))
            .map(img => {
              const dataSrc = img.getAttribute('data-lazy') || img.getAttribute('data-src');
              const src = img.getAttribute('src');
              const srcset = img.getAttribute('data-srcset');
              const url = dataSrc || srcset?.split(',')[0]?.trim()?.split(' ')[0] || src;
              console.log('Image trouvée:', { dataSrc, src, srcset, url });
              return url;
            })
            .filter(Boolean);
          ...
          logElement('price', priceEl, priceContainerEl);
          logElement('capacity', capacityEl, capacityContainerEl);
