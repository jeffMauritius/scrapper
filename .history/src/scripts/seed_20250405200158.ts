...
async function main() {
  try {
    ...
    for (const venue of data.venues) {
      ...
      const maxCapacity = venue.capacity ? parseInt(venue.capacity.match(/\d+/)?.[0] || '0') : 0;
      ...
      try {
        ...
        console.log(`Créé établissement: ${establishment.name}`);
        console.log(`Images ajoutées: ${venue.images.length}`);
      } catch (error) {
