...
async function scrapeVenuePage(url: string): Promise<VenueData | null> {
  ...
  try {
    ...
    const price = parseInt(priceText as string, 10) || 0;
    console.log('Price:', price);

    const address = await page.$eval('.venue-address', (el: Element) => el.textContent?.trim() || '');
    console.log('Address:', address);

    const capacityText = await page.$eval('.venue-capacity', (el: Element) => el.textContent?.trim() || '');
    const capacity = parseInt(capacityText as string, 10) || 0;
    console.log('Capacity:', capacity);

    const rating = await page.$eval('.venue-rating', (el: Element) => {
      const ratingText = el.textContent?.trim() || '0';
