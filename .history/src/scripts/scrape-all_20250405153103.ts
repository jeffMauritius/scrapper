...
async function scrapeListPage(page: Page, baseUrl: string, searchParams: string): Promise<string[]> {
  ...
  try {
    await page.waitForSelector('.app-list-directory-item', { timeout: 60000 });
    console.log('Sélecteur trouvé, extraction des liens...');

    // Extraire les URLs de la page courante
    const pageLinks = await page.$$eval('.app-list-directory-item a.app-list-directory-item-content', (elements) => {
      return elements
