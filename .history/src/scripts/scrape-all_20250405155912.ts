...
async function getFullDescription(page: Page): Promise<string> {
  try {
    ...
    // Récupérer la description complète directement de la page détaillée
...
async function scrapeListPage(page: Page): Promise<VenueData[]> {
  ...
  try {
    ...
    for (const card of cards) {
      try {
        // Extraire l'URL et le nom avant de cliquer
        ...
        if (cardData.url) {
          console.log(`\nExtraction des détails pour ${cardData.name}...`);
          
          // Ouvrir la page détaillée dans un nouvel onglet
          const newPage = await page.context().newPage();
          try {
            await newPage.goto(cardData.url, PAGE_OPTIONS);
            await newPage.waitForLoadState('networkidle');
            await newPage.waitForTimeout(2000);

            // Extraire toutes les informations de la page détaillée
            const detailData = await newPage.evaluate(() => {
              const nameEl = document.querySelector('h1.storefrontHeading__title');
              const ratingEl = document.querySelector('.storefrontHeading__rating strong');
              const reviewsEl = document.querySelector('.storefrontHeading__rating .rating-counter');
              const locationEl = document.querySelector('.storefrontHeading__address');
              const priceEl = document.querySelector('.storefrontHeading__price');
              const capacityEl = document.querySelector('.storefrontHeading__capacity');
            ...
            const description = await getFullDescription(newPage);

            // Récupérer les images
            const images = await newPage.$$eval('.gallery-thumbs img, .gallery-slider img', (imgs) => {
              return imgs
            ...
            const [city, region] = detailData.location.split(',').map(s => s.trim());
            const type = determineVenueType(cardData.url);

            const venue: VenueData = {
              url: cardData.url,
              name: detailData.name || cardData.name,
              type,
              description,
              images,
              ...
              rating: `${detailData.rating} (${detailData.reviews})`
            };
